...
| 工作负载 | 输出 | 当前 API，ms | C++ 直接入口，ms | 保留 R 参数检查，ms | 直接入口加速 | 保留检查加速 |
|---|---|---:|---:|---:|---:|---:|
| 单对结构（阴性） | have | 4.480 | 0.070 | 2.340 | 64.00× | 1.91× |
| 单对结构（阳性） | have | 2.360 | 0.070 | 1.100 | 33.71× | 2.15× |
| 512 结构 × 25 motif | have | 476.000 | 27.000 | 79.000 | 17.63× | 6.03× |
| 512 结构 × 25 motif | count | 513.000 | 29.000 | 81.000 | 17.69× | 6.33× |
| 512 结构 × 25 motif | match | 513.000 | 29.000 | 81.000 | 17.69× | 6.33× |
| 4,096 行（64 个独立结构）× 25 motif | have | 111.000 | 6.000 | 30.000 | 18.50× | 3.70× |
| 8,009 结构 × 13 motif | have | 7415.000 | 277.000 | 1475.000 | 26.77× | 5.03× |
...
- 共 **704,925 次结构对 × 输出类型比较**，132 个 workload/输出检查条目，
