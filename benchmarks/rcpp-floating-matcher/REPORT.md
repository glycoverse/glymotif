...
- 另有 128 × 25 普通结构三种输出的 9,600 次回归比较通过；
  对普通及浮动输入序列化前后核对，确认 C++ 调用没有修改输入对象。
...
全量 R 基线三轮为 238.062、258.633、309.958 秒，呈逐轮变慢趋势；
本实验未单独诊断原因。C++ 直接入口范围为 0.180–0.219 秒，
保留检查为 0.228–0.316 秒。因此加速倍数仅描述本次测量，不应视为稳定的生产承诺。
这是单机、这些结构和 motif 的测量，不是所有复杂度或硬件上的速度保证。
全部计时在主要浮动正确性核验完成后独立执行。
