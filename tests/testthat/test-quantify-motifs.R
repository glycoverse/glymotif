test_that("quantify_motifs works in a most common glycoproteomics situation", {
  ...
  expect_equal(result$meta_data$exp_type, "traitproteomics")
})
...
test_that("quantify_motifs works with character vector of glycan structures", {
  # ----- Create an experiment for test -----
  # Create expression matrix
  expr_mat <- matrix(
    c(1, 2, 1,
...
test_that("quantify_motifs works for glycoproteomics data with glycan annotations", {
  # ----- Create an experiment for test -----
  # Create expression matrix
  expr_mat <- matrix(
    c(1, 2, 1,
  ...
  var_info <- tibble::tibble(
    ...
    has_fuc = c(FALSE, FALSE, TRUE)  # this column should be dropped
  )
