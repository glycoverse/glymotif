...
Use predefined motif names in the GlyGen GlycoMotif database (https://glycomotif.glyomics.org/glycomotif/GGM) instead:
...
You might think: "It's just pattern matching, right?"
Well, not quite.
...
Therefore, if you try to match a "topological" glycan against a "intact" motif,
you will get all negative results.
...
Previously, we mentioned that you can use motif names in the GlycoMotif database (https://glycomotif.glyomics.org/glycomotif/).
A common task is to match your glycans against a packaged motif collection.
By default, `db_motifs()` uses the GlyGen Motifs collection (`source_id = "GGM"`) for backward compatibility:
...
Motifs in the built-in database have various alignments,
which are automatically configured when using `db_motifs()` for `motifs`.
This also means that you cannot set arguments like `alignments` when using `db_motifs()`.

```{r}
try(have_motifs(glycans, db_motifs(), alignments = "substructure"))
```

You can use `source_id` to select another motif collection.
Use `db_motif_info()` to browse all built-in motifs,
and `dplyr::distinct(db_motif_info(), source_id, source)` to list all available sources.
...
- [StrucGAP](https://github.com/Sun-GlycoLab/StrucGAP): A comprehensive Python platform for glycoproteomics analysis
